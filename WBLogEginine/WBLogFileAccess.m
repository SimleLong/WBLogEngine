...
@implementation WBLogFileAccess
...
- (instancetype)init
{
    self = [super init];
    if (self) {
        _io_queue = dispatch_queue_create("logfile_io_queue", 0);
        [self buildFolderPath:[self logCacheDir] error:nil];        
    }
    return self;
}
